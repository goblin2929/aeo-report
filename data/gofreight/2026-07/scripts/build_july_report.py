...
G = json.load(open('d:/tmp/gofreight-july-data.json', encoding='utf-8'))
WD = json.load(open('d:/tmp/wd_july.json', encoding='utf-8'))
PC = json.load(open('d:/tmp/gf-page-clicks-july.json', encoding='utf-8'))
CORE = json.load(open('d:/tmp/gf-core-page-us-july.json', encoding='utf-8'))
GA4 = json.load(open('d:/tmp/ga4_ai_traffic.json', encoding='utf-8'))
DROP = json.load(open('d:/tmp/ga4-ai-drop.json', encoding='utf-8'))
W31 = json.load(open('d:/tmp/wd_w31.json', encoding='utf-8'))
RW = json.load(open('d:/tmp/recent_work_map.json', encoding='utf-8'))
CG = json.load(open('d:/tmp/ga4-chatgpt-lp.json', encoding='utf-8'))   # ChatGPT-only landing pages
...
# ---- top-line (GSC + CSV dashboard) ----
CL_JUN,CL_JUL=4796,6390
IM_JUN,IM_JUL=1295126,1247565
CTR_JUN,CTR_JUL=0.37,0.51
POS_JUN,POS_JUL=10.2,10.3
AIS_JUN,AIS_JUL=480,392            # GA4 direct
VIS_JUN,VIS_JUL=23.3,29.3          # WorkDuo dashboard (CSV, +6.0pp)
SOV_JUN,SOV_JUL=21.5,26.0          # WorkDuo dashboard (CSV, +4.5pp)
LLMENG_JUN,LLMENG_JUL=215,137      # WorkDuo LLM Engaged Sessions (CSV, -78)
...
wser=[w for w in G['weekly'] if '2026-01-05'<=w['week']<='2026-07-27']
...
wv=WD['weekly_visibility']; wv['2026-W31']=W31
jj_weeks=['2026-W23','2026-W24','2026-W25','2026-W26','2026-W27','2026-W28','2026-W29','2026-W30','2026-W31']
...
def aeo_mom_rows():
    ...
    return f'''<tr><td>AI Visibility</td><td class="num">{VIS_JUN:.1f}%</td><td class="num">{VIS_JUL:.1f}%</td><td class="num up">+{VIS_JUL-VIS_JUN:.1f} pts</td><td>WorkDuo dashboard</td></tr>
<tr><td>Share of Voice</td><td class="num">{SOV_JUN:.1f}%</td><td class="num">{SOV_JUL:.1f}%</td><td class="num up">+{SOV_JUL-SOV_JUN:.1f} pts</td><td>WorkDuo dashboard</td></tr>
<tr><td>AI Platform Sessions (GA4)</td><td class="num">{f(AIS_JUN)}</td><td class="num">{f(AIS_JUL)}</td><td class="num down">{(AIS_JUL-AIS_JUN)/AIS_JUN*100:.1f}%</td><td>GA4 direct</td></tr>
<tr><td>LLM Engaged Sessions</td><td class="num">{f(LLMENG_JUN)}</td><td class="num">{f(LLMENG_JUL)}</td><td class="num down">{sgn(LLMENG_JUL-LLMENG_JUN)}</td><td>WorkDuo dashboard</td></tr>
...
pickle.dump(frag,open('d:/tmp/frag_july.pkl','wb'))
print("wrote frag_july.pkl; weeks:",len(week_labels))
